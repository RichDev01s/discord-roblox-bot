...
client.once("clientReady", () => {
  ...
  console.log(`🔗 Invite: https://discord.com/api/oauth2/authorize?client_id=${client.user.id}&permissions=277025393664&scope=bot`);
});
...
async function handleGenServer(message, gameKey) {
  ...
  try {
    ...
    try {
      ...
      await loading.edit({ content: `${game.emoji} ¡Servidor encontrado! Te lo envié por DM 📬` });
    } catch {
      await loading.edit({ content: `${game.emoji} No pude enviarte un DM. Activa los mensajes directos e intenta de nuevo.` });
    }
  } catch (err) {
